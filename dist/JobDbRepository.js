...
class JobDbRepository {
    ...
    async lockJob(job) {
        ...
        const update = { $set: { lockedAt: new Date(), lastModifiedBy: this.agenda.attrs.name } };
        const options = {
    ...
    async getNextJobToRun(jobName, nextScanAt, lockDeadline, now = new Date()) {
         ...
         */
        const JOB_PROCESS_SET_QUERY = { $set: { lockedAt: now, lastModifiedBy: this.agenda.attrs.name } };
        /**
